...
describe("Twitter Contract", function () {
  ...
  // before each unit test, it will just go ahead and do this thing
  ...
  // first test: lets test the add tweet
  describe("Add Thoughts", function () {
  ...
  describe("Get All Thoughts", function () {
    it("should return the correct number of total thoughts", async function () {
    ...
    it("should return the correct number of all my thoughts", async function () {
      const myTweetsFromChain = await twitter.getMyTweets();
      expect(myTweetsFromChain.length).to.equal(NUM_TOTAL_MY_TWEETS);
    })
  })

  describe("Delete Thoughts", function () {
    it("should emit delete thought event", async function () {
      const TWEET_ID = 0;
